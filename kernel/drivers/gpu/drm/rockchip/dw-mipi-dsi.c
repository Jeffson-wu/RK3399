...
static void dw_mipi_dsi_encoder_enable(struct drm_encoder *encoder)
{
    ...
    gpio_set_value(54, 1);
	/* SN65DSI84 */
	dev_info(dsi->dev, "DHYOON: I2C START\n" );
	adapter = i2c_get_adapter(4);
	if (!adapter) {
	    dev_info(dsi->dev, "DHYOON: ERR I2CAA\n" );
		pr_err("%s: fail to get bridge adapter\n", __func__);
//		return -ENODEV;
	}

	dev_info(dsi->dev, "DHYOON: I2C NEWDEVICE\n" );
	if (i2c_new_device(adapter, &iircade_device) == NULL) {
		i2c_put_adapter(adapter);
	    dev_info(dsi->dev, "DHYOON: ERR I2C BB\n" );
		pr_err("%s: fail to create a bridge adapter\n", __func__);
//		return -ENODEV;
	}

	dev_info(dsi->dev, "DHYOON: I2C ADD DRV\n" );
	i2c_add_driver(&sn65dsi84_driver);
	i2c_del_driver(&sn65dsi84_driver);
	dev_info(dsi->dev, "DHYOON: I2C END\n" );
#endif
	if (dsi->dphy.phy)
