...
/* do a hack for getting the system volume */
static int sys_vol = 1;

int sys_setvol(int vol)
{
	sys_vol = vol;
	return 1;
}
 ...
 */
//static int pmdown_time = 1728000000;
static int pmdown_time = 10;
...
#include <linux/delay.h>
/* gpio work */
static struct delayed_work spk_power_work;
static void spk_power_on(void *arg)
{

	(void)arg;
	
	gpio_set_value(153, 1); // SPK_SD
	msleep(200);
	if(sys_vol == 0)
		gpio_set_value(158, 0); // SPK_MUTE
	else
		gpio_set_value(158, 1); // SPK_MUTE
}

static void spk_power_off(void *arg)
{
	(void)arg;

    	gpio_set_value(158, 0); // SPK_MUTE
	msleep(200);
	gpio_set_value(153, 0); // SPK_SD
}

static int spk_power(struct snd_soc_dapm_widget *w,
			    struct snd_kcontrol *k, int event)
{
	int on = !!SND_SOC_DAPM_EVENT_ON(event);
	static int __first_init__ = 1;

	if (!on)
		return spk_power_off(NULL), 0;

	if (__first_init__)
	{
		INIT_DELAYED_WORK(&spk_power_work, (work_func_t)spk_power_off);
		schedule_delayed_work(&spk_power_work, msecs_to_jiffies(50));
		__first_init__ = 0;
		return 0;
	}

	return spk_power_on(NULL), 0;
}

static const struct snd_soc_dapm_widget simple_widgets[] = {
	SND_SOC_DAPM_MIC("Microphone", NULL),
	SND_SOC_DAPM_LINE("Line", NULL),
	SND_SOC_DAPM_HP("Headphone", spk_power),
	SND_SOC_DAPM_SPK("Speaker", spk_power),
};
