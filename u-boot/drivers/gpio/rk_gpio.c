...
static int rockchip_gpio_probe(struct udevice *dev)
{
	...
	printf("###############ydh1234 rockchip_gpio_probe!!!\n");
	rockchip_gpio_direction_output(dev, 150, 1);	// 
	rockchip_gpio_set_value(dev, 150, 1);
	rockchip_gpio_direction_output(dev, 154, 1);	// backlight on
	rockchip_gpio_set_value(dev, 154, 0);
	rockchip_gpio_direction_output(dev, 54, 0);	// M2L_EN off
	rockchip_gpio_set_value(dev, 54, 0);
/*	mdelay(200);
	rockchip_gpio_direction_output(dev, 151, 0);	// backlight off
	rockchip_gpio_set_value(dev, 151, 0);
	mdelay(200);
	rockchip_gpio_direction_output(dev, 151, 1);	// backlight on
	rockchip_gpio_set_value(dev, 151, 1);
	mdelay(200);
	rockchip_gpio_direction_output(dev, 151, 0);	// backlight off
	rockchip_gpio_set_value(dev, 151, 0);
	rockchip_gpio_set_value(dev, 150, 0);
	mdelay(200);
*/
	return 0;
}
